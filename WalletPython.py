...
os.chdir(r"C:\Users\NgWei\OneDrive\Documents\Python Scripts\Wallet")
...
def NewListing(bot,update,user_data):
    ...
    curList[userid] = ListId
    bot.edit_message_text(message_id=query.message.message_id,chat_id = query.message.chat_id,
                          text = "ListID =" + str(userid) + str(curList[userid]) + "\nType:" + query.data + "\nPlease give us a listing title")
    ListObj = Listing(query.data)
    save_object(ListObj,List_dir + '\\' + ListId + '.pkl')
    return CREATE3
     
def AddTitle(bot,update):
    global curList
    user = update.message.from_user
    userid = user.id
    print(userid)
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
                ...
                CREATE3: [MessageHandler(Filters.text,AddTitle)]
